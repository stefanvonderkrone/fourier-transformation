...
package de.stefanvonderkrone.as3.fourier.tests {
    ...
    public class FFT2DNonRecursiveStaticTest extends TestSuite {

        private var _bitmapReals : Vector.<Vector.<Number>>;
        private var _bitmapImags : Vector.<Vector.<Number>>;

        public function FFT2DNonRecursiveStaticTest() {
            name = "2D-FFT-Test";
            ...
            tests = [
                ...
                new MethodTest( fft, [ _bitmapReals[8], _bitmapImags[ 8 ] ], "size 2048x2048", 20, 1, "" )
            ];
        }
        ...
        private static function fft( reals : Vector.<Number>, imags : Vector.<Number> ):void {
            reals = reals.concat();
            imags = imags.concat();
            const size : int = Math.sqrt( reals.length );
            FFT2DNonRecursive.transform( new Point( size, size ), reals, imags );
        }
